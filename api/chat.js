...
export default async function handler(req, res) {
  ...
  if (success && finalData) {
    ...
    async function geolocateIp(ip) {
      if (!ip) return null;
      try {
        const r = await fetch(`http://ip-api.com/json/${encodeURIComponent(ip)}?fields=status,message,lat,lon,city,regionName,country,query`);
    ...
    // Dapatkan IP & geolokasi (jika tersedia)
    const clientIp = getClientIp(req);
    const geo = await geolocateIp(clientIp).catch(() => null);

    // Ambil teks jawaban dari struktur response
    let answerText = '';
    try {
      if (finalData.output && typeof finalData.output === 'string') answerText = finalData.output;
      else if (finalData.candidates && finalData.candidates[0] && finalData.candidates[0].content) answerText = finalData.candidates[0].content;
      else answerText = JSON.stringify(finalData).slice(0, 2000);
    } catch (e) {
      answerText = JSON.stringify(finalData).slice(0, 2000);
    }
    ...
    const payload = {
      ...
      country: geo?.country,
      question: typeof contents === 'string' ? contents : JSON.stringify(contents),
      answer: answerText
    };

    // Fire-and-forget forwarding to external webhook (set WEBHOOK_URL env to e.g. https://logswormhistory.vercel.app/api/logs)
    try {
      ...
      if (webhook) {
        fetch(webhook, {
          ...
          if (!r.ok) console.warn('Webhook forward failed', r.status);
        }).catch(e => console.error('Webhook forward error', e));
      }
    } catch (e) {
      console.error('forward try error', e);
    }
